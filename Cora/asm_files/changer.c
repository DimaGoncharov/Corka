...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/12/21 17:39:40 by ptorchbu          #+#    #+#             */
/*   Updated: 2019/12/21 17:39:44 by ptorchbu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int			comment_cleaner(t_data *data)
{
	...
	if ((cleaned_file =
		...
		ft_out(data, "Cleaned_file malloc error\n");
...
static int			file_trimer(t_data *data, int *nb_lines, int i)
{
	...
	if ((trimmed_file =
		...
		ft_out(data, "Trimmed_file malloc error\n");
	while (j < *nb_lines)
	{
...
static int			sharp_cleaner(t_data *data, int *nb_lines)
{
	...
	if ((sharp_cleaned =
		(char **)ft_memalloc(sizeof(char *) * *nb_lines)) == NULL)
		ft_out(data, "Sharp_cleaned malloc error\n");
	while (i < *nb_lines)
	{
...
int					changer(t_data *data, int nb_lines)
{
	...
	if ((i = ft_manag_head(data)) == 0)
		ft_out(data, "Header error\n");
