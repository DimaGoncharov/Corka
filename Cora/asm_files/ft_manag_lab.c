...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/12/21 17:41:20 by ptorchbu          #+#    #+#             */
/*   Updated: 2019/12/21 17:41:23 by ptorchbu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		label_check(int *nb_label, t_data *data, int i)
{
	...
	k = 0;
	while (ft_isspace(data->file[i][k]) || ft_label_char(data->file[i][k]))
	{
		k++;
		if (data->file[i][k] == ':')
		{
...
static int		label_to_line(t_var *var, t_data *data)
{
	...
	while (ft_isspace(data->file[var->i][var->k]) ||
		ft_label_char(data->file[var->i][var->k]))
	{
		var->k++;
...
static int		label_cleaner(t_data *data, int nb_label)
{
	...
	if ((var.label_cleaned = (char **)ft_memalloc(sizeof(char *) *
		...
		ft_out(data, "label_cleaned malloc error\n");
